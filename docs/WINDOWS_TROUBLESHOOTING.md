...
### 問題1: ライブラリがインストールされているのに読み込めない

**症状:**
- プラグイン（weather, bongo catなど）がインストールされ、依存関係のインストールも成功している
- しかし、プラグインを有効化すると「ライブラリが見つかりません」というエラーが出る
- `python -m pip list`で確認するとパッケージはインストールされている

**原因:**
`--user`オプションでインストールされたパッケージが、ユーザーsite-packagesディレクトリに配置されますが、Horloqの実行環境でそのパスが読み込まれていない可能性があります。

**解決方法:**

#### 自動修正（v0.7.1以降）
v0.7.1以降では、この問題は自動的に修正されます。アップデートしてください。

#### 手動での確認
もし問題が続く場合は、以下のコマンドでパッケージの場所を確認してください：

```cmd
python -c "import site; print(site.getusersitepackages())"
```

出力されたパスに実際にパッケージがインストールされているか確認：
```cmd
dir "C:\Users\<ユーザー名>\AppData\Roaming\Python\Python311\site-packages"
```

#### 代替インストール方法
システム全体にインストールする（管理者権限が必要）：
```cmd
python -m pip install requests pynput pillow --force-reinstall
```

### 問題2: 権限エラー（Permission Error）
